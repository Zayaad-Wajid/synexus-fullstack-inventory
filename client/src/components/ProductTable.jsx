...
function ProductTable({ products, onEdit, onDelete, isLoading, canDelete = false }) {
  if (isLoading) {
    return (
                ...
                <td className="px-4 py-4 align-top">
                  <div className="flex justify-end gap-2">
                    ...
                    <button
                      ...
                      disabled={!canDelete}
                      title={canDelete ? "Delete product" : "ADMIN access is required to delete products"}
                      className="rounded-md border border-red-200 px-3 py-1.5 text-xs font-semibold text-red-700 transition hover:bg-red-50 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-50 disabled:text-slate-400"
                    >
                      Delete
                    </button>
                  </div>
